...
export const TRACKER_VCS_WORKFLOW_BOUNDARY = workflowBoundary("vcs");
export const trackerVcsWorkflowScaffoldSummary = workflowBoundarySummary("vcs");
...
// ── Milestone helpers ──────────────────────────────────────────────────
// NOTE: This block is mirrored in .pi/extensions/vazir-tracker/vcs.ts.
...
function syncFromFossil(cwd: string): void {
  ...
  try {
    const changes = childProcess.execSync("fossil changes", { cwd, encoding: "utf-8", stdio: "pipe", timeout: 5000 }).trim();
  ...
  for (const [file, status] of statusMap) {
    ...
    if (status === "A") {
      ...
      try {
        const diff = childProcess.execFileSync("fossil", ["diff", "--", file], { cwd, encoding: "utf-8", stdio: "pipe", timeout: 5000 });
...
function fossilRefLabel(cwd: string): string {
  try {
    const branch = childProcess.execSync("fossil branch current", { cwd, encoding: "utf-8", stdio: "pipe", timeout: 5000 }).trim();
  ...
  try {
    const info = childProcess.execSync("fossil info", { cwd, encoding: "utf-8", stdio: "pipe", timeout: 5000 });
...
function fossilAutosyncEnabled(cwd: string): boolean | null {
  try {
    const value = childProcess.execSync("fossil setting autosync", { cwd, encoding: "utf-8", stdio: "pipe", timeout: 5000 }).trim().toLowerCase();
...
export function syncChanges(cwd: string, vcsKind: VcsKind, invalidateStoryProgressCache?: (cwd: string) => void): VcsDisplayInfo {
