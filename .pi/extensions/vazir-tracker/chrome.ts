...
const VAZIR_COMMAND_HELP: CommandHelpEntry[] = [
  ...
  { command: "/test-sandbox", description: "ask the agent to plan and run a natural-language test in a disposable workspace" },
...
const VAZIR_COMMAND_DOCS: CommandDoc[] = [
  ...
  },
  {
    ...
    shortDesc: "ask the agent to plan and run a natural-language test in a disposable workspace",
    usage: "/test-sandbox [request]",
    args: ["request — optional natural-language behavior to test; Vazir prompts when omitted"],
    examples: ["/test-sandbox test the new checkpoint restore flow", "/test-sandbox verify story-088 command cancellation"],
    longDesc: "Hands a natural-language request to the agent, which inspects the project and active story and decides whether workspace isolation is appropriate. For an appropriate request, the agent calls Vazir's sandbox tool with a purpose-specific structured plan. Vazir previews every executable and argument, states the security boundary, and requires explicit approval before running. Results include phase evidence, bounded log excerpts, and preserved failure paths. Recommended before /complete-story, but never automatic or a completion gate. Workspace isolation is not host security isolation.",
  },
  {
    command: "/fix",
...
const VAZIR_QUICKSTART_MARKDOWN = [
  ...
  "4. `/test-sandbox [request]` lets the agent derive a story-specific test plan, previews every command for approval, and runs it in a disposable workspace. This is recommended before closeout.",
  "5. `/complete-story` checks readiness, optionally runs review, and closes the story when it is truly done. Sandbox testing is opt-in, not a completion gate.",
  "6. `/idea [description]` captures a thought in `.context/ideas/`; bare `/idea` lets you capture or browse ideas.",
  "",
