...
- When normalising external inputs, reuse the canonical helper at every consumption site; do not re-implement partial extracts that drift from the canonical fallback logic. <!-- source: story-028 --> <!-- confidence: high -->
- Do not silently swallow exceptions from structural persistence operations; surface a warning so users know when downstream undo or recovery actions will be unavailable. <!-- source: story-028 --> <!-- confidence: high -->
### From successes
