...
### Problem summary
E2E tests need a repeatable disposable application workspace so generated files, test mutations, ports, and background services do not interfere with the real project. This first version is workspace isolation only: same-user test processes remain able to access host files, processes, environment values, credentials, and the network.

### Proposed behavior/spec
- Add an explicit `/test-sandbox` workflow; do not invoke it automatically from review, implementation, fix, or story-closeout flows.
- Persist stack-neutral setup, start, readiness, and test commands in `.context/settings/project.json` as structured executable-and-argument arrays rather than shell strings.
...
- Reuse deterministic loopback port allocation, bound every phase, terminate spawned process trees, and capture phase-specific logs and outcomes.
- Clean successful workspaces by default, optionally preserve failures for inspection, and never export changes into the source project automatically.
...
| story-088 | Add the explicit `/test-sandbox` workflow | not-started | story-085, story-086, story-087 |
